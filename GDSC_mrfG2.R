...
X1 <- GDSC$x[,GDSC$num.nonpen+which(colnames(GDSC$x)[GDSC$num.nonpen+1:p[1]] %in% 
                                      name_genes)]
p[1] <- ncol(X1)
X1 <- log2(X1)
...
#diag_idx1 <- cbind(rep(Idx_Pathway1,each=length(2:5)) + rep((2:5-1)*sum(p),times=length(Idx_Pathway1)),
                   rep(Idx_Pathway1,each=length(2:5)) + rep((2:5-1)*sum(p),times=length(Idx_Pathway1)))
#Gmrf_Group1Pathway1 <-rbind(Gmrf_Group1Pathway1, diag_idx1)
...
#diag_idx2 <- cbind(rep(Idx_Pathway2,each=length(6:7)) + rep((6:7-1)*sum(p),times=length(Idx_Pathway2)),
                   rep(Idx_Pathway2,each=length(6:7)) + rep((6:7-1)*sum(p),times=length(Idx_Pathway2)))
#Gmrf_Group2Pathway2 <- rbind(Gmrf_Group2Pathway2, diag_idx2)
...
example_GDSC$mrfG <- rbind(  Gmrf_Group1Pathway1, Gmrf_Group2Pathway2, Gmrf_CommonGene )
GDSC_mrfG2 <- example_GDSC
save(GDSC_mrfG2, file="GDSC_mrfG2.rda") 
