...
#===============
# The user needs load the datasets from https://www.cancerrxgene.org release 5. 
...
#var.x1 <- apply(log2(x[,num.nonpen+1:p[1]]), 2, var)
var.x1 <- apply(log(x[,num.nonpen+1:p[1]]), 2, var)
var.sort <- sort(var.x1, decreasing=TRUE)
sum.a <- cumsum(var.sort)
half.a <- var.sort[which(sum.a>(sum.a[length(var.x1)]*0.1))[1]]
x1 <- x[,num.nonpen+1:p[1]][,var.x1>=half.a]
#x1 <- x[,num.nonpen+1:p[1]]

# name.exTarget <- colnames(x[,num.nonpen+1:p[1]])[grep("MAP2K2|MAP2K1|MAPK1|MAPK2|RAF", colnames(x[,num.nonpen+1:p[1]]))]
# ## add deleted targeted genes into x1
# x1 <- cbind(x1, x[,name.exTarget])
...
save(GDSC, file="GDSC_BayesSUR.rda")
...
# delete gene ABL2
Idx_Pathway2 <- which(name_genes_duplicate %like% "BCR" | name_genes_duplicate %like% "ABL")[c(4,9,11,13,14)] # for top 100% cumVar[GEX]
...
save(example_GDSC_kegg, file="example_GDSC_kegg10%.RData")
...
save(targetGene, file="targetGene_kegg_10%.RData")
